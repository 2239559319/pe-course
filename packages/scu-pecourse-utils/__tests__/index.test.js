import { app_key, app_secret, getSign } from '../src';
...
describe('test sign', () => {
  ...
  it('get sign test', () => {
    expect(getSign(getData.path, app_secret, getData.timestamp)).toBe(
      getData.sign
    );
  });
  it('post sign test', () => {
    expect(
      getSign(postData.path, app_secret, postData.timestamp, postData.data)
    ).toBe(postData.sign);
  });
});
